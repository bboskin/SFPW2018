...
(define (stop₁-expr v)
  (or (var? v)
      (and (pair? v) (memv (car v) '(same λ cons)))))

(define (not-var? v) (not (var? v)))

(defrel (check Γ expr τ o)
  (conde
   [(check-cons Γ expr τ o)] ;; ΣI
   [(check-λ Γ expr τ o)] ;; FunI-1
   [(check-= Γ expr τ o)] ;; EqI
   [(symbolo expr) (switch-expr Γ expr τ o)] ;; Switch
   [(fresh (a d)
      (== expr `(,a . ,d))
      (=/= a 'same)
      (=/= a 'λ)
      (=/= a 'cons))
    (switch-expr Γ expr τ o)])) ;; Switch
...
(defrel (type Γ τ o)
  (conde
    ...
    [(type-= Γ τ o)] ;; EQ-F
    [(non-reserved-Pie-symbol τ) (check Γ τ 'U o)]
    [(fresh (a d)
       (== τ `(,a . ,d))
       (=/= a 'Σ)
       (=/= a 'Π)
...
(defrel (≡-type Γ e1 e2)
  (conde
    ...
    [(non-reserved-Pie-symbol e1) (≡ Γ 'U e1 e2)]
    [(fresh (a d)
       (== `(,a . ,d) e1)
       (=/= a 'Σ)
       (=/= a 'Π)
